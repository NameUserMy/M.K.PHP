...
function getHotels(mysqli $mysqli) {

	$hotels=array();
	$mysqli->real_query("SELECT id,hotel FROM hotels");
...
function getComment(mysqli $mysqli,int $hotelId) {

	$comments=array();
	$mysqli->real_query("SELECT * FROM comments Where hotelId=$hotelId");
	$rows = $mysqli->use_result();
	foreach ($rows as $key => $value) {
